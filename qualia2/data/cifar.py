...
class CIFAR10(ImageLoader):
    ...
    def __init__(self, normalize=True, flatten=False):
        ...
        for i in range(5):
            self.train_data[i*10000:(i+1)*10000] = self._load_data(path + '/download/cifar10/cifar-10.tar.gz', i+1, 'train')
            self.train_label[i*10000:(i+1)*10000] = CIFAR10.to_one_hot(self._load_label(path + '/download/cifar10/cifar-10.tar.gz', i+1, 'train'), 10)

        self.test_data = self._load_data(path + '/download/cifar10/cifar-10.tar.gz', i+1, 'test')
        self.test_label = CIFAR10.to_one_hot(self._load_label(path + '/download/cifar10/cifar-10.tar.gz', i+1, 'test'), 10)
        print('[*] done.')
...
class CIFAR100(ImageLoader):
    ...
    def __init__(self, normalize=True, flatten=False):
        ...
        self.train_data = self._load_data(path + '/download/cifar100/cifar-100.tar.gz', 'train')
        self.train_label = CIFAR100.to_one_hot(self._load_label(path + '/download/cifar100/cifar-100.tar.gz', 'train'), 100)

        self.test_data = self._load_data(path + '/download/cifar100/cifar-100.tar.gz', 'test')
        self.test_label = CIFAR100.to_one_hot(self._load_label(path + '/download/cifar100/cifar-100.tar.gz', 'test'), 100)
        print('[*] done.')
